...
def _sm_basis_from_sun_and_m(s_sun: np.ndarray, m_hat: np.ndarray) -> np.ndarray:
    ...
    # --- DEGENERACY GUARD: s ~ // m ---
    # If ||x|| ~ 0, choose any direction perpendicular to m
...
class SolarGeomagneticTransformer:
    ...
    def sm_ll_to_ecef(self,
        ...
        - sm_lat_rad, sm_lon_rad: scalar / 1D / 2D (e.g., Ny×Nx grid)
        - r_norm: scalar or broadcastable array to shape sm_lat/sm_lon
        - when: epoch (astropy Time or string), one for all points
        Returns xyz with shape (..., 3) consistent with the input.
        """
    ...
    def ecef_to_geodetic(self, xyz_m: ArrayLike) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
        """
        ECEF (...,3) -> geodetic (lat[rad], lon[rad], h[m]) ..
        """
